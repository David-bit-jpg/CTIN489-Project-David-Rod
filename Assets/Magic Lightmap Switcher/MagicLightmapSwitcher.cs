...
namespace MagicLightmapSwitcher
{
    ...
    public class MagicLightmapSwitcher : MonoBehaviour
    {
        ...
        public class AffectedObject
        {
            ...
            {
                if (!Application.isPlaying && _propBlock != null)
                {
                    if (meshRenderer != null)
                    {
        ...
        private ComputeShader lightprobesBlending;
        public BlendingOptions currentBlandingState;
        ...
        public class SceneLightingPreset
        {
            ...
            [SerializeField]
            public List<LightSourceSettings> lightSourcesSettings = new List<LightSourceSettings>();
            [SerializeField]
            ...
            public void MatchSceneSettings()
            {
                ...
                {                    
                    LightSourceSettings currentSettings = lightSourcesSettings.Find(item => item.light == sceneLights[i]);
                ...
                if (skyboxSettings.bakerySkyLightsSettings != null)
                {
                    if (skyboxSettings.bakerySkyLightsSettings.bakerySky != null)
                    {
                        ...
                        skyboxSettings.bakerySkyLightsSettings.bakerySky.transform.rotation = skyboxSettings.bakerySkyLightsSettings.cubemapAngles;
            ...
            public void UpdatePresetData()
            {
                ...
                for (int i = 0; i < sceneLights.Length; i++)
                {
                    ...
                    if (currentSettings != null)
                    {
                        ...
                        currentSettings.tempRotation = TransformUtils.GetInspectorRotation(sceneLights[i].transform);
                ...
                if (skyboxSettings.bakerySkyLightsSettings != null && skyboxSettings.bakerySkyLightsSettings.bakerySky != null)
                {
                    ...
                    skyboxSettings.bakerySkyLightsSettings.cubemapAngles = skyboxSettings.bakerySkyLightsSettings.bakerySky.transform.rotation;
                }
        ...
        public List<StoredLightmapData> UpdateLightmapData(string tagetSceneName)
        {
            ...
            tempLightmapDataList.AddRange(Resources.LoadAll<StoredLightmapData>(FileUtil.GetProjectRelativePath(systemProperties.storePath + "/" + tagetSceneName)));

            for (int i = 0; i < tempLightmapDataList.Count; i++)
            {
                if (tempLightmapDataList[i].workflow == workflow)
                {
                    tempLightmapDataList[i].storedReflectionProbesData = new Dictionary<string, Cubemap>();

                    for (int j = 0; j < tempLightmapDataList[i].sceneLightingData.reflectionProbes.name.Length; j++)
                    {
                        tempLightmapDataList[i].storedReflectionProbesData.Add(
                    ...
                    tempLightmapDataList[i].rendererDatasDeserialized = new Dictionary<string, StoredLightmapData.RendererData>();

                    for (int j = 0; j < tempLightmapDataList[i].sceneLightingData.rendererDatas.Length; j++)
                    {
                        tempLightmapDataList[i].rendererDatasDeserialized.Add(tempLightmapDataList[i].sceneLightingData.rendererDatas[j].objectId, tempLightmapDataList[i].sceneLightingData.rendererDatas[j]);
                    }

                    tempLightmapDataList[i].terrainDatasDeserialized = new Dictionary<string, StoredLightmapData.TerrainData>();

                    for (int j = 0; j < tempLightmapDataList[i].sceneLightingData.terrainDatas.Length; j++)
                    {
                        tempLightmapDataList[i].terrainDatasDeserialized.Add(tempLightmapDataList[i].sceneLightingData.terrainDatas[j].objectId, tempLightmapDataList[i].sceneLightingData.terrainDatas[j]);
                    }

                    tempLightmapDataList[i].lightSourceDataDeserialized = new Dictionary<string, StoredLightmapData.LightSourceData>();

                    for (int j = 0; j < tempLightmapDataList[i].sceneLightingData.lightSourceDatas.Length; j++)
                    {
                        tempLightmapDataList[i].lightSourceDataDeserialized.Add(tempLightmapDataList[i].sceneLightingData.lightSourceDatas[j].lightUID, tempLightmapDataList[i].sceneLightingData.lightSourceDatas[j]);
                    }
        ...
        public List<StoredLightingScenario> UpdateLightingScenarios(string targetScene, bool forceUpdateStoredData = false)
        {
            ...
            tempLightmapScenariosList.AddRange(Resources.LoadAll<StoredLightingScenario>(systemProperties.storePath + "/" + targetScene));

            if (tempLightmapScenariosList.Count > 0)
            {                
                Blending.blendingOperationalDatas.TryGetValue(targetScene, out blendingOperationalData);
        ...
        public IEnumerator UpdateStoredArray(string targetScene, bool forceUpdateStoredData = false)
        {
            ...
            if (lastSceneCount != SceneManager.sceneCount || forceUpdateStoredData)
            {
                ...
                bool removeOrAdd = false;

                if (lastSceneCount > SceneManager.sceneCount)
                {
                    removeOrAdd = false;
                }
                else if (lastSceneCount < SceneManager.sceneCount)
                {
                    removeOrAdd = true;
                }
                ...
                switch (workflow)
                {
                    case Workflow.MultiScene:
                        if (removeOrAdd || forceUpdateStoredData)
                        {
                            ...
                            {
                                storedLightmapDatas.Add(targetScene, UpdateLightmapData(targetScene));
                            }
                            else
                            {
                                storedLightmapDatas[targetScene] = UpdateLightmapData(targetScene);
                        ...
                        {
                            List<StoredLightingScenario> sourceScenariosSet;

                            storedLightmapScenarios.TryGetValue(targetScene, out sourceScenariosSet);
                        ...
                        break;
                    case Workflow.SingleScene:
                        sceneLightmapDatas = UpdateLightmapData(targetScene);
                        ...
                        sceneReflectionProbes = new List<ReflectionProbe>(FindObjectsOfType<ReflectionProbe>());
                        break;
                }                
            }
        ...
        {
            currentBlandingState = blendingOptions;
            ...
            for (int i = 0; i < sceneDynamicAffectedObjects.Count; i++)
            {
                ...
                switch (blendingOptions)
                {
                    ...
                    case BlendingOptions.None:
                        ...
                        sceneDynamicAffectedObjects[i].SetShaderTexture(Blending._MLS_SkyReflection_Blend_From, defaultCubeBlack);
                        sceneDynamicAffectedObjects[i].SetShaderTexture(Blending._MLS_SkyReflection_Blend_To, defaultCubeBlack);
                        break;
                }
        ...
        private void OnEnable()
        {
            ...
            if (workflow == Workflow.SingleScene)
            {
        ...
        private static void CreateDefaultCubemap()
        {
#if UNITY_EDITOR
        ...
        private void FindSystemProperies()
        {
            ...
            {
                string[] directories = Directory.GetDirectories(Application.dataPath, "Magic Lightmap Switcher", SearchOption.AllDirectories);
                string projectRelativePath = Application.dataPath + directories[0].Split(new string[] { "Magic Lightmap Switcher" }, System.StringSplitOptions.None)[1];
        ...
        private void LoadDependentAssets()
        {
            ...
            if (systemProperties == null)
            {
                workPath = "";
                FindSystemProperies();
                Debug.LogFormat("<color=cyan>MLS:</color> Plugin's position in the project hierarchy has changed.");
            }
#endif

            if (lightprobesBlending == null)
            {
                lightprobesBlending = Resources.Load<ComputeShader>("Magic Lightmap Switcher/Shaders/LightprobesBlending");

#if UNITY_EDITOR
                if (lightprobesBlending == null)
                {
                    string targetDirectory = Application.dataPath + "/Resources/Magic Lightmap Switcher/Shaders";
                    ...
                    if (!File.Exists(Application.dataPath + "/Resources/Magic Lightmap Switcher/Shaders/LightprobesBlending.compute"))
                    {
                        File.Copy(workPath + "/Editor/Dependent Resources/LightprobesBlending.compute",
                            Application.dataPath + "/Resources/Magic Lightmap Switcher/Shaders/LightprobesBlending.compute");
                    }
                }
#endif
            }
