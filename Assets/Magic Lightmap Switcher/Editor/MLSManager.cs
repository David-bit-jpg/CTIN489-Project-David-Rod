    ...
    public class MLSManager : EditorWindow
    {
        ...
        public static void ClearAllPluginData()
        {
                ...
                "in the scene and removing all saved lightmaps as well as scenarios. This action is irreversible. Proceed? ", "Yes", "No"))
            {
                ...
                MLSControlledLight[] mLSControlledLights = FindObjectsOfType<MLSControlledLight>();

                objectsToDestroyCount = mLSControlledLights.Length;

                for (int i = 0; i < objectsToDestroyCount; i++)
                {
                    DestroyImmediate(mLSControlledLights[i]);
                }
        ...
        public static void CheckState()
        {
            ...
            {
                if (activeSwitcherInstance.currentBlandingState != MagicLightmapSwitcher.BlendingOptions.None)
                {
        ...
        void OnGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox("New version (" + EditorPrefs.GetString("MLS_LITE_latestVersion") + ") " +
                    "available.\r\nGo to [Tools->Magic Tools->Magic Lightmap Switcher->About MLS...] to download it.", MessageType.Warning, true);
            }
                        ...
                        using (new GUILayout.VerticalScope(GUI.skin.box))
                        {
                            ...
                            switch (activeSwitcherInstance.storingMode)
                            {
                                ...
                                case MagicLightmapSwitcher.StoringMode.Queue:
                                    ...
                                    using (new GUILayout.HorizontalScope())
                                    {
                                        GUILayout.Label(MLSTooltipManager.MainComponent.GetParameter("Store Path", MLSTooltipManager.MainComponent.Tabs.Storing), GUILayout.MinWidth(180));
                                        GUILayout.FlexibleSpace();
                                        GUILayout.Label("Resources/", GUILayout.MinWidth(50));
                                        activeSwitcherInstance.systemProperties.storePath = EditorGUILayout.TextField(activeSwitcherInstance.systemProperties.storePath, GUILayout.MaxWidth(240));

                                        if (string.IsNullOrEmpty(activeSwitcherInstance.systemProperties.storePath))
                                        {
                                            activeSwitcherInstance.systemProperties.storePath = "MLS_DATA";
                                        }

                                        activeSwitcherInstance.systemProperties.storePath = activeSwitcherInstance.systemProperties.storePath.Trim(new char[] { '/' });
                                    }
                                    break;
                            }
                        }
            ...
            using (new GUILayout.VerticalScope(GUI.skin.box))
            {
                ...
                using (new GUILayout.VerticalScope(GUI.skin.box))
                {
                            ...
                            if (MLSEditorUtils.DrawHeader(scenarios[i].prefix + scenarios[i].scenarioName, true, new Color(0.3f, 0.3f, 0.3f, 1), scenarios, i))
                            {
                                ...
                                using (new GUILayout.VerticalScope(GUI.skin.box))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (scenarios[i].blendableLightmaps.Count != scenarios[i].lastBlendableLightmapsCount || scenarios[i].collectedCustomBlendableDatas.Count == 0)
                                        {
                                            ...
                                            if (scenarios[i].reorderableStoredLightmapDatas.count > 0)
                                            {
                                                ...
                                                { 
                                                    ...
                                                    if (!activeSwitcherInstance.systemProperties.useSwitchingOnly &&
                                                        (activeSwitcherInstance.systemProperties.standardRPActive && !activeSwitcherInstance.systemProperties.standardRPPatched) ||
                                                        (activeSwitcherInstance.systemProperties.universalRPActive && !activeSwitcherInstance.systemProperties.universalRPPatched) ||
                                                        (activeSwitcherInstance.systemProperties.highDefinitionRPActive && !activeSwitcherInstance.systemProperties.highDefinitionRPPatched))
                                                    {
                    ...
                    else
                    {
                        ...
                        activeInstance.StartCoroutine(activeInstance.UpdateStoredArray(EditorSceneManager.GetSceneAt(selectedScene).name, true));
                    }
                }
            }
        }
