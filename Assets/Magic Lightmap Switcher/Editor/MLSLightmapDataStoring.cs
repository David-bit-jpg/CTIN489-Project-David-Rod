...
namespace MagicLightmapSwitcher
{
    public static class MLSLightmapDataStoring
    {
        ...
        private static IEnumerator BakeAndStoreLightmapQueue(MagicLightmapSwitcher activeSwitcherInstance, bool createLightmapScenario)
        {
            ...
            for (int i = 0; i < activeSwitcherInstance.lightingPresets.Count; i++)
            {
                ...
                switch (currentLightmapper)
                {
                    ...
                    case MagicLightmapSwitcher.Lightmapper.BakeryLightmapper:
                        ...
                        ftRenderLightmap.instance.RenderReflectionProbesButton();
                        ...
                        if (LightmapSettings.lightProbes != null && LightmapSettings.lightProbes.count > 0)
                        {
                            ...
                            {
                                ftRenderLightmap.instance.RenderLightProbesButton();
            ...
            if (!stopBakingQueue)
            {
                if (createLightmapScenario)
                {
                    ...
                    {
                        MLSManager.RemoveStoredLightingScenario(MLSManager.scenarioToChange - 1);
        ...
        public static IEnumerator StoreLightingData(MagicLightmapSwitcher activeSwitcherInstance, string lightmapName, bool manualBaking = false)
        {
            ...
            string sceneLightmapDataPath =
                Application.dataPath + "/Resources/" + activeSwitcherInstance.systemProperties.storePath + "/" + EditorSceneManager.GetActiveScene().name;
            ...
            if (File.Exists(FileUtil.GetProjectRelativePath(
                sceneLightmapDataPath + "/" + EditorSceneManager.GetActiveScene().name + "_" + lightmapName + ".asset")))
            {
                File.Delete(FileUtil.GetProjectRelativePath(
                sceneLightmapDataPath + "/" + EditorSceneManager.GetActiveScene().name + "_" + lightmapName + ".asset"));
            }

            lightmapDataAsset = Editor.CreateInstance<StoredLightmapData>();
            AssetDatabase.CreateAsset(lightmapDataAsset, FileUtil.GetProjectRelativePath(
                sceneLightmapDataPath + "/" + EditorSceneManager.GetActiveScene().name + "_" + lightmapName + ".asset"));
        ...
        private static void CheckIfScenarioExist(MagicLightmapSwitcher lightmapSwitcher, string name, List<StoredLightmapData> storedLightmapData = null)
        {
            string sceneLightmapDataPath = 
                Application.dataPath + "/Resources/" + activeSwitcherInstance.systemProperties.storePath + "/" + EditorSceneManager.GetActiveScene().name;
            ...
            StoredLightingScenario storedLightingScenario = Resources.Load<StoredLightingScenario>(sceneLightmapDataPath + "/" + prefix + name);

            if (storedLightingScenario != null)
            {
                if (EditorUtility.DisplayDialog("Magic Lightmap Switcher", "A preset with this name has already been created.", "OK"))
                {
                    return;
                }
            }
        ...
        public static void CreateLightmapScenario(MagicLightmapSwitcher lightmapSwitcher, string name, List<StoredLightmapData> storedLightmapData = null)
        {
            ...
            if (storedLightingScenario == null)
            {
                ...
                AssetDatabase.CreateAsset(storedLightingScenario, FileUtil.GetProjectRelativePath(sceneLightmapDataPath + "/" + prefix + name + ".asset"));
                AssetDatabase.SaveAssets();
                AssetDatabase.Refresh();
