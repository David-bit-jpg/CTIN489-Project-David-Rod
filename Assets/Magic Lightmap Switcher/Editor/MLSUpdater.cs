...
namespace MagicLightmapSwitcher
{
    ...
    {         
        ...
        public static string installedVersion = "1.37.3";
        ...
        private static IEnumerator CheckForUpdatesEnumerator()
        {
            ...
            if (System.DateTime.Now.Hour != EditorPrefs.GetInt("MLS_LITE_lastCheck") || forceCheck)
            {
                forceCheck = false;
                EditorPrefs.SetInt("MLS_LITE_lastCheck", System.DateTime.Now.Hour);
            }
            else
            {
                yield break;
            ...
            using (UnityWebRequest checkUpdate = UnityWebRequest.Post("https://www.motiongamesstudio.com/mls-update/checkForUpdates.php", form))
            {
                ...
                else
                {
                    ...
                    {
                        EditorPrefs.SetBool("MLS_LITE_newVersionAvailable", true);
                        EditorPrefs.SetString("MLS_LITE_installedVersion", installedVersion);
                        EditorPrefs.SetString("MLS_LITE_latestVersion", checkUpdate.GetResponseHeader("Latest-Version"));
                    } 
                    else
                    {
                        EditorPrefs.SetBool("MLS_LITE_newVersionAvailable", false);
                        EditorPrefs.SetString("MLS_LITE_installedVersion", installedVersion);
                    }
                }
            }
        ...
        private static IEnumerator DownloadUpdateEnumerator(string uName, string uInvoice)
        {
            ...
            form.Add("installedVersion", EditorPrefs.GetString("MLS_LITE_installedVersion"));
            ...
            using (UnityWebRequest download = UnityWebRequest.Post("https://www.motiongamesstudio.com/mls-update/checkForUpdates.php", form))
            {
                ...
                else
                {
                    if (download.GetResponseHeader("Content-Description") == "File Transfer")
                    {
                        ...
                        EditorPrefs.SetString("MLS_LITE_uName", uName);
                        EditorPrefs.SetString("MLS_LITE_uInvoice", uInvoice);
                        EditorPrefs.SetBool("MLS_LITE_Authorized", true);
