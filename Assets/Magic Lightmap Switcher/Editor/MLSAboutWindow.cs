...
namespace MagicLightmapSwitcher
{
    public class MLSAboutWindow : EditorWindow
    {
        ...
        static void Init()
        {
            if (EditorPrefs.GetBool("MLS_LITE_Authorized"))
            {
        ...
        void OnGUI()
        {
            ...
            if (EditorPrefs.GetBool("MLS_LITE_newVersionAvailable"))
            {
                GUILayout.Label("Version " + EditorPrefs.GetString("MLS_LITE_latestVersion") + " Available", greenLabelStyle);
            }
            else
            {
            ...
            if (EditorPrefs.GetBool("MLS_LITE_newVersionAvailable"))
            {
                if (!MLSUpdater.authorization)
                {
                    if (GUILayout.Button("Download New Version"))
                    {
                        if (EditorPrefs.GetBool("MLS_LITE_Authorized"))
                        {
                            MLSUpdater.StartDownload(EditorPrefs.GetString("MLS_LITE_uName"), EditorPrefs.GetString("MLS_LITE_uInvoice"));
                        }
                        else
                        {
