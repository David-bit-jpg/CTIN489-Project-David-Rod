...
float3 _GlobalVolumeMin, _GlobalVolumeInvSize;
    ...
    // Bicubic
    float4 MLS_BakeryTex2D(int textId, float2 uv, float4 texelSize)
    {
        float x = uv.x * texelSize.z;
        float y = uv.y * texelSize.z;
        ...
        return
            BakeryBicubic_g0(fy) * (g0x * BlendBakeryRNM(textId, (float2(px + h0x, py + h0y) * texelSize.x)) +
    ...
    float4 MLS_BakeryTex2D(int textId, SamplerState s, float2 uv, float4 texelSize)
    {
        float x = uv.x * texelSize.z;
        float y = uv.y * texelSize.z;
        ...
        return     BakeryBicubic_g0(fy) * (g0x * BlendBakeryRNMSampler(textId, (float2(px + h0x, py + h0y) * texelSize.x), s) +
            g1x * BlendBakeryRNMSampler(textId, (float2(px + h1x, py + h0y) * texelSize.x), s)) +

            BakeryBicubic_g1(fy) * (g0x * BlendBakeryRNMSampler(textId, (float2(px + h0x, py + h1y) * texelSize.x), s) +
                g1x * BlendBakeryRNMSampler(textId, (float2(px + h1x, py + h1y) * texelSize.x), s));
    }
#endif
#else
    // Bilinear
    ...
    float4 MLS_BakeryTex2D(int textId, float2 uv, float4 texelSize)
    {
        return BlendBakeryRNM(textId, uv);
    }

    float4 MLS_BakeryTex2D(int textId, SamplerState s, float2 uv, float4 texelSize)
    {
        return BlendBakeryRNMSampler(textId, uv, s);
    }
#endif
#endif
...
{
#if defined(MLS_BLENDING_STANDARD) || defined(MLS_BLENDING_SRP)
    float3 dominantDir = lerp(UNITY_SAMPLE_TEX2D_SAMPLER(unity_LightmapInd, unity_Lightmap, lmUV).xyz * 2 - 1, BlendTwoTextures(1, lmUV, 0).xyz * 2 - 1, _MLS_ENABLE_LIGHTMAPS_BLENDING);    
#else
...
#if defined(MLS_BLENDING_STANDARD) || defined(MLS_BLENDING_SRP)
    float3 rnm0 = lerp(DecodeLightmap(BakeryTex2D(_RNM0, lmUV, _RNM0_TexelSize)), DecodeLightmap(MLS_BakeryTex2D(0, lmUV, _RNM0_TexelSize)), _MLS_ENABLE_LIGHTMAPS_BLENDING);
    float3 rnm1 = lerp(DecodeLightmap(BakeryTex2D(_RNM1, lmUV, _RNM0_TexelSize)), DecodeLightmap(MLS_BakeryTex2D(1, lmUV, _RNM0_TexelSize)), _MLS_ENABLE_LIGHTMAPS_BLENDING);
    float3 rnm2 = lerp(DecodeLightmap(BakeryTex2D(_RNM2, lmUV, _RNM0_TexelSize)), DecodeLightmap(MLS_BakeryTex2D(2, lmUV, _RNM0_TexelSize)), _MLS_ENABLE_LIGHTMAPS_BLENDING);
#else
...
void BakerySH(inout float3 diffuseColor, inout float3 specularColor, float2 lmUV, float3 normalWorld, float3 viewDir, float smoothness)
{
    ...
    float3 L0 = lerp(DecodeLightmap(BakeryTex2D(unity_Lightmap, samplerunity_Lightmap, lmUV, _RNM0_TexelSize)), DecodeLightmap(MLS_BakeryTex2D(3, lmUV, _RNM0_TexelSize)), _MLS_ENABLE_LIGHTMAPS_BLENDING);
#else
    float3 L0 = lerp(DecodeLightmap(UNITY_SAMPLE_TEX2D(unity_Lightmap, lmUV)), DecodeLightmap(BlendTwoTextures(0, lmUV)), _MLS_ENABLE_LIGHTMAPS_BLENDING);
#endif
    float3 nL1x = lerp(BakeryTex2D(_RNM0, lmUV, _RNM0_TexelSize) * 2 - 1, MLS_BakeryTex2D(0, lmUV, _RNM0_TexelSize) * 2 - 1, _MLS_ENABLE_LIGHTMAPS_BLENDING);
    float3 nL1y = lerp(BakeryTex2D(_RNM1, lmUV, _RNM0_TexelSize) * 2 - 1, MLS_BakeryTex2D(1, lmUV, _RNM0_TexelSize) * 2 - 1, _MLS_ENABLE_LIGHTMAPS_BLENDING);
    float3 nL1z = lerp(BakeryTex2D(_RNM2, lmUV, _RNM0_TexelSize) * 2 - 1, MLS_BakeryTex2D(2, lmUV, _RNM0_TexelSize) * 2 - 1, _MLS_ENABLE_LIGHTMAPS_BLENDING);
    float3 L1x = nL1x * L0 * 2;
    float3 L1y = nL1y * L0 * 2;
    float3 L1z = nL1z * L0 * 2;
#else
    ...
    float3 nL1y = BakeryTex2D(_RNM1, lmUV, _RNM1_TexelSize) * 2 - 1;
    float3 nL1z = BakeryTex2D(_RNM2, lmUV, _RNM2_TexelSize) * 2 - 1;
    float3 L1x = nL1x * L0 * 2;
    float3 L1y = nL1y * L0 * 2;
    float3 L1z = nL1z * L0 * 2;
#endif
